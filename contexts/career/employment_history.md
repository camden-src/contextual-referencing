# Employment History Reference
**Last Updated:** December 11, 2025

**Purpose:** Authoritative reference for job titles, companies, dates, and employment context. Use this to ensure consistency across application materials.

---

## PROFESSIONAL EMPLOYMENT HISTORY

### Novaprime LLC
**Location:** Remote  
**Role:** Staff Infrastructure Engineer  
**Dates:** March 2024 – September 2024
**Industry:** Mortgage Origination SaaS featuring ML quality controls and blockchain signing of mortgage document custodianship

**Context:**
- First full-time infrastructure hire (not first hire overall)
- Inherited existing AWS infrastructure from contractors
- Professionalized and transformed contractor work into production-grade systems
- Supported distributed remote team

**Key Responsibilities:**
- Cloud infrastructure (AWS, Cloudflare, Azure, DataBricks)
- CI/CD pipelines (GitHub Actions, FluxCD)
- Monitoring and observability (New Relic, CloudWatch, PagerDuty)
- Automation tooling (Python, Go)
- Participating in leadership meetings

**Team Structure:**
- One less senior team (participated in hiring process in Dover)
- Collaborated with engineering team and contractors
- Technical leadership role for infrastructure

**Titles Held:**
- Staff Infrastructure Engineer

---

### Gro Intelligence
**Location:** Hybrid, New York, NY  
**Role:** Staff Software Engineer, Infrastructure  
**Dates:** August 2023 – March 2024 (8 months)  
**Industry:** Climate Intelligence, Agricultural Data, AI-powered Analytics  

**Context:**
- Hired to replace geographically based team lead
- Women-led climate technology company
- AI-powered platform for agricultural and climate data

**Key Responsibilities:**
- Infrastructure modernization (AWS, EKS, Terraform)
- CI/CD redesign (Python OOP architecture replacing bash scripts)
- DevSecOps integration with Security Engineering team
- DevOps best practices and documentation

**Team Structure:**
- 2 junior team members
- Championed infrastructure practices organization-wide
- Mentored junior team members on infrastructure tools and code refactoring

**Special Notes:**
- Delivered Terraform modules for EKS with FluxCD (handoff incomplete)
- Light exposure to Ansible (cannot speak to concepts deeply)
- Supported Digital Asset Canton blockchain deployment

---

### Roivant Sciences
**Location:** New York, NY  
**Roles:**
- DevOps Engineer: February 2019 - June 2020
- Software Engineering Manager, Vant Technology: June 2020 - January 2022
- Infrastructure Engineer - DevOps: January 2022 - November 2022
**Dates:** February 2019 – November 2022
**Industry:** Pharmaceutical R&D, Biotechnology  

**Context:**
- Pharmaceutical research and development organization
- Mission-critical infrastructure supporting drug development
- SOC2 compliance and regulatory environment
- Platform supporting 20-25 non-engineering users
- Most business functions adopted "Vant" as part of their unit's name

**Key Responsibilities:**
- Cloud infrastructure (GCP - GKE, Pub/Sub, BigQuery)
- CI/CD automation (GitLab CI, Terraform)
- Container orchestration (GKE, Helm, ArgoCD)
- Monitoring and uptime protection (99.9% uptime)
- Security and compliance (SOC2, Zero Trust, HashiCorp Vault)
- Identity/access management (Okta integration, JWT/SAML)

**Team Structure:**
- Multiple non-concuurent team assignments:
  - Mid-level member of Plaform Engineering Team
  - Team-level manager of a team of 8 engineers called "Vant Technology"
  - Infrastructure Engineering team member
- Collaborated extensively with Security and Compliance teams on all assignments

**Special Notes:**
- Kafka/MSK production experience
- Google Cloud Pub/Sub production experience
- Okta cross-domain authentication and tokenization
- Pharmaceutical regulatory compliance environment

---

### Metropolitan Museum of Art
**Location:** New York, NY  
**Role:** Software Development Contractor  
**Dates:** July 2018 – February 2019 
**Industry:** Cultural Institution, Digital Infrastructure  

**Context:**
- Contract software development role
- Museum's digital infrastructure projects

**Key Responsibilities:**
- Go-based web services development
- Backend APIs for digital infrastructure
- Data pipelines and third-party integrations
- Content management and analytics systems

**Team Structure:**
- Contractor role
- Collaborated with museum's technology team

**Special Notes:**
- Prototyped but did not implement ELK stack

---

### Getty Images
**Location:** New York, NY  
**Roles:** Senior Software Engineer  
**Dates:** 
- July 2012 – July 2017
- November 2008 - January 2012
 
**Industry:** Digital Media, Content Delivery, High-Scale SaaS  

**Context:**
- High-scale digital media platform
- Serving millions of users globally
- High availability requirements

**Key Responsibilities:**
- Backend infrastructure (Mesos/Marathon, Consul)
- CI/CD systems (Jenkins, Docker)
- Service mesh and microservices architecture
- Observability platform (Splunk)
- Messaging (RabbitMQ)

**Team Structure:**
- Senior software engineer bridging into DevOps
- Technical mentorship for junior engineers
- Established deployment patterns and best practices

**Notable Accomlishments:**
- Rebuilt a mission critical editorial event coverage system from a Microsoft AccessDB customization to an enterprise-hosted web-delivered management application. Delivered ahead of schedule.
- Built and optimized a high volume editorial image subscription distribution system. Able to get images from camera to customer within 5 minutes under high volume loads encountered during key events, such as sports championships, award shows, and notable news events.

**Special Notes:**
- Pre-Kubernetes era distributed systems (Mesos/Marathon)
- Enterprise Splunk deployment (machine configuration, agent)
- MongoDB exposure (long ago, not current expertise)

---

### Boomerang Systems
**Location:** Florham Park, NJ
**Role:** Software Engineering Team Lead
**Dates:** 
- January 2012 – July 2012
 
**Industry:** Robotic Parking Garages

**Context:**
- engineering team geographically diverse
- principal engineer located remotely within US
- associated with other larger successful engineering projects

**Key Responsibilities:**
- Backend on-premises office infrastructure (Windows Server Racks)
- Hardware network construction (cabling and patch bay configuration)
- Access Controls (Active Directory, application enforced)
- SDLC Framework(TFS)
- Team standards, technical, operational, and procedural
- Communication and collaboration with the offshore team

**Team Structure:**
- Team lead of 11 engineers spanning coding, database, integration, and testing disciplines

**Special Notes:**
- short term tenure, measurable results not available

## CAREER GAPS & INTENTIONAL BREAKS

### Gap: July 2017 – July 2018 (12 months)
**Reason:** Intentional career gap for family time and creative work (artist and parent)  
**Context:** Prioritized personal projects and family responsibilities

### Gap: February 2019 – July 2018 (overlaps with Met Museum contract)
**Reason:** Transition period between Getty Images and Roivant Sciences  
**Context:** Contract work at Metropolitan Museum of Art during transition

### Gap: September 2024 – Present
**Reason:** Job search, targeted applications to mission-aligned companies  
**Context:** Selective about opportunities, prioritizing mission-driven organizations

---

## CAREER TRAJECTORY NOTES

### Progression Pattern
- **2012-2017:** Senior Software Engineer (5 years at Getty)
- **2018-2019:** Contract Software Developer (Met Museum)
- **2019-2022:** DevOps Engineer, Team Lead (4 years at Roivant)
- **2023-2024:** Staff-level Infrastructure roles (Gro, Novaprime)

### Role Evolution
- Started in backend development and infrastructure
- Evolved into DevOps/Infrastructure specialization
- Progressed to team leadership (Roivant)
- Staff-level individual contributor roles (Gro, Novaprime)

### Company Size Preference
- **Getty Images:** Large, established company (~1000+ employees)
- **Roivant Sciences:** Mid-size organization (~200-500 employees)
- **Gro Intelligence:** Startup (~50-100 employees)
- **Novaprime:** Small company (~10-50 employees)
- **Pattern:** Increasingly drawn to smaller, mission-driven organizations

---

## TOTAL EXPERIENCE CALCULATION

### Years of Professional Experience
- **Overall:** 13+ years (2012 - 2025, accounting for gaps)
- **DevOps/Infrastructure focus:** 13+ years (backend infrastructure since Getty 2012)
- **AWS:** 10+ years (since ~2014-2015)
- **Kubernetes:** 5+ years production (since ~2019 at Roivant)
- **Terraform:** 5+ years (since ~2019 at Roivant)
- **Team Leadership:** 4 years (Roivant 2019-2022, leading 8 engineers)

---

## COMPANY CONTEXT & CULTURES

### Gro Intelligence (Women-Led Organization)
- Women-led climate technology company
- Felt pride in supporting women-led organization
- Climate and agricultural intelligence mission

### Roivant Sciences (Pharmaceutical R&D)
- Regulatory compliance environment (SOC2)
- Mission-critical systems impacting drug development
- Collaborative security and compliance culture
- Led team through organizational change

### Getty Images (High-Scale SaaS)
- Millions of users, high availability requirements
- Pre-Kubernetes distributed systems era
- Enterprise observability and monitoring

---

## EDUCATION

**Institution:** Drew University, Madison, NJ  
**Degree:** Bachelor of Arts (BA)  
**Field:** Not Computer Science (liberal arts degree)  
**Years:** ~2008-2012 (approximate, confirm exact dates if needed)

**Important Note:**  
- Degree is BA, NOT BS/BE in Computer Science
- Acceptable for "BS or equivalent experience" requirements
- 13+ years professional experience demonstrates equivalent expertise

---

## CURRENT STATUS

**As of December 2025:**
- **Status:** Actively job searching
- **Focus:** Mission-driven organizations (healthcare, sustainability, social impact)
- **Approach:** Targeted applications, not spray-and-pray
- **Preferences:** Remote-first or flexible work arrangements
- **Last Role:** Novaprime (ended September 2024)
- **Last Salary:** $180,000 (reference point, not shared in applications)

---

## RESUME FORMATTING PREFERENCES

### Contact Information
- **Name:** Camden Lindahl
- **Location:** Blauvelt, NY (Eastern Time Zone)
- **Phone:** (718) 644-0407
- **Email:** camden.lindahl@gmail.com
- **LinkedIn:** linkedin.com/in/camden-lindahl
- **Portfolio:** camdenwander.com

### Job Title Variations
Comfortable with various titles depending on role:
- DevOps Engineer
- Staff Infrastructure Engineer
- Platform Engineer
- Site Reliability Engineer (SRE)
- Cloud Infrastructure Engineer
- Lead DevOps Engineer

---

## REFERENCES & VERIFICATIONS

**Reference Check Notes:**
- All employment dates and titles are verifiable
- Intentional career gaps should be explained positively (family/creative time)
- First full-time hire at Novaprime (not first hire overall) - inherited contractor work
- Team lead at Roivant (8 direct reports)

---

## REVISION HISTORY

**December 11, 2025:** Initial employment history reference document created
- Split from master reference document for GitHub publishing
- Focused on factual employment dates, titles, and company context
- Removed personal preferences and technical details (separate documents)
